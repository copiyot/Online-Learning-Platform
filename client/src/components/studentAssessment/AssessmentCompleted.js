...
class AssessmentCompleted extends React.Component {
  ...
  componentWillMount() {
    let submittedAssessments = this.props.assessments.filter(
      (assign) => `${assign.submitted}` === "true"
    );

    this.setState({ assessments: submittedAssessments });
  }
...
export default connect(mapStateToProps)(AssessmentCompleted);

...
class AssignmentPastDue extends React.Component {
  state = { assignments: [], id: "", selected: "assignment-list" };

  componentWillMount() {
    let dueAssignments = this.props.assignment.filter(
      (assign) =>
        new Date() > new Date(assign.dateExpected) &&
        `${assign.submitted}` !== "true"
    );

    this.setState({ assignments: dueAssignments });
  }
...
export default connect(mapStateToProps)(AssignmentPastDue);

...
class AssignmentCompleted extends React.Component {
  state = { assignments: [], id: "", selected: "assignment-list" };

  componentWillMount() {
    let submittedAssignments = this.props.assignment.filter(
      (assign) => `${assign.submitted}` === "true"
    );

    this.setState({ assignments: submittedAssignments });
  }
  ...
  render() {
    const { selected, id, assignments } = this.state;
...
export default connect(mapStateToProps)(AssignmentCompleted);

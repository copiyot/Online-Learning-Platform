...
import "../createSchemeOfWork/CreateSchemeOfWork.css";
import { createSchemeOfWork, fetchSchemeOfWork } from "../../actions";
import { BASE_URL } from "../api";
import Axios from "axios";

class EditSchemeOfWork extends Component {
  state = {
    subjects: [],
    selectedSubject: "",
    term: "",
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const { createdBy, description, subjectId, term, id } = nextProps.scheme;

    this.setState({ selectedSubject: subjectId, description, term, id });
  ...
  onSubmitHandler = async () => {
    const { id, selectedSubject, description, term, formErrors } = this.state;
    ...
    if (this.formValid(formErrors)) {
      ...
      const response = await Axios.put(
        BASE_URL + "curriculum/scheme_of_work/update",
        data
      );
